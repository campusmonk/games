...
insert into public.daily_limit_settings (id, games_per_day, communication_per_day, debug_per_day)
values ('global', null, null, null)
on conflict (id) do nothing;

create table if not exists public.daily_limit_usage (
  email text not null,
  kind text not null check (kind in ('game', 'communication', 'debug') or kind like 'game:%' or kind like 'communication:%' or kind like 'debug:%'),
...
check (kind in ('game', 'communication', 'debug') or kind like 'game:%' or kind like 'communication:%' or kind like 'debug:%');
